...
- Drag operations are strictly prevented at boundary edges - objects cannot cross boundaries
- Objects are constrained during creation and movement
...
- Visual indicator: Different border color and reduced opacity for locked objects
...
- Clear visual feedback when attempting to move a locked object (different border color + opacity change)
